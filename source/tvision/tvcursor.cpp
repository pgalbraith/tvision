...
// decideCaretSize is the start/end pair held in TScreen::cursorLines, not
// the single percentage the 32-bit target uses.
...
void TView::resetCursor()
{
    ...
    dosSetCaretSize(caretSize, Boolean(state & sfCursorIns));
#else
    if (caretSize)
